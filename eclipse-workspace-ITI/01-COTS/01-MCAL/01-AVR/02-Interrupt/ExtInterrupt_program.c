...
#include <ExternalInterrupt_private.h>
...
{
	switch(Copy_u8Index){
		case INTERRUPT_INT0:clr_bit(GICR,6);break;
		case INTERRUPT_INT1:clr_bit(GICR,7); break;
		case INTERRUPT_INT2:clr_bit(GICR,5);break;
		}
}
u8 Interrupt_setLevel(u8 Copy_u8Index,u8 Copy_u8Level){
	u8 Local_u8ErrorState=0;
	if (Copy_u8Index == 2 && (Copy_u8Level == INTERRUPT_LEVEL_ANY_LOGICAL_CHANGE ||Copy_u8Level == INTERRUPT_LEVEL_LOW_LEVEL)){
		Local_u8ErrorState=1;
		return Local_u8ErrorState;
	}
	else {
	switch(Copy_u8Index){
			case INTERRUPT_INT0:assign_2bits(MCUCR,0,Copy_u8Level);break;
			case INTERRUPT_INT1:assign_2bits(MCUCR,2,Copy_u8Level); break;
			case INTERRUPT_INT2:assign_2bits(MCUCSR,6,Copy_u8Level & 0b01);break;
			}
	}
	return Local_u8ErrorState;
}
void Interrupt_voidSetCallback(u8 Copy_u8Index,void (*Copy_ptr)(void)){
	switch(Copy_u8Index){
				case INTERRUPT_INT0:PvoidCallback[0] = Copy_ptr;break;
				case INTERRUPT_INT1:PvoidCallback[1] = Copy_ptr; break;
				case INTERRUPT_INT2:PvoidCallback[2] = Copy_ptr;break;
